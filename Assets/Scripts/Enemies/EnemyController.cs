...
public class EnemyController : MonoBehaviour {
    ...
    public void SetStats(float threatLevel, float difficulty) {
        attack = (int)Math.Round((attack * (Mathf.Clamp(threatLevel / 2, 1, 2)) * difficulty), MidpointRounding.AwayFromZero);
        health = (int)Math.Round((health * (Mathf.Clamp(threatLevel / 2, 1, 2)) * difficulty), MidpointRounding.AwayFromZero);
    }
